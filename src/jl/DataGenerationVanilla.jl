...
function data_generation_main()
    ...
    output_file   = conf.output_file_ram
    ...
    for it in 1:iteration_num
        ...
        # --- Sample feasible unit commitment
        print("Sampling feasible unit commitment for iteration $it... ", G, T, u0, U0, L0, Ton, Toff, "\n")
        u0_int = Int.(u0)
        U0_int = Int.(U0)
        L0_int = Int.(L0)
        Ton_int = Int.(Ton)
        ...
        u = sample_feasible_u(G, T, u0_int, U0_int, L0_int, Ton_int, Toff_int)
        println("Sampled feasible unit commitment: ", u)

        # --- Model ---
        model = Model(Gurobi.Optimizer)
        set_optimizer_attribute(model, "OutputFlag", 0)

        # --- Randomly generate wind power scenarios ---
        Pw = []
        Pw_encoder = []
        for i in 1:W
            ...
            @info "Wind farm $i: Generated wind power scenarios with shape $(size(tmp))"
