...
output_file = conf.output_file_opt * "_mean.json"
...
if debug
    @info "---- Parameter Logging ----"
    @info "T (number of time steps): $T"
    @info "Gval (number of units): $Gval"
    @info "G: $G"
    @info "N (number of buses): $N"
    @info "W (number of wind farms): $W"
    @info "S_w (scenarios per wind farm): $S_w"
    @info "S (number of uncertainty scenarios): $S"
    @info "Gbus: $Gbus"
    @info "Dbus: $Dbus"
    @info "Wbus: $Wbus"
    @info "Pramp: $Pramp"
    ...
    @info "Lupv: $Lupv"
    @info "Llow: $Llow"
    @info "LN_ori (first entry): $(LN_ori[1])"
    @info "---------------------------"
end
...
for it in 1:iteration_num
    ...
    @info "$it iteration's Objective value: $scd_obj"
    @info "$it iteration's Objective value: $first_obj"
